...
- **Anomalies Table:** Results from the ML model are stored here. It identifies _which_ asset had _what_ problem.
...
He will ask: _"How does the code talk to the database?"_

- **MySQL Connector:** used in `app/app.py`. We use `cursor.execute(SQL_QUERY)` to fetch data.
- **Hybrid Logic (Vercel vs Local):**
  - _Question:_ "Why does your Vercel link work without MySQL?"
  - _Answer:_ I implemented an `IS_VERCEL` check. If true, it uses **Mock Data** (Simulated data) so the UI doesn't crash. If false (Local), it connects to the actual MySQL DB.
- **Dynamic Routing:** The `@app.route('/analysis/<int:asset_id>')` uses a variable in the URL to fetch data for a _specific_ asset from the DB.
...
5.  **"How do you prevent SQL Injection?"**
    - By using _Parameterized Queries_ (e.g., `cursor.execute(query, (params,))`) instead of f-strings.
